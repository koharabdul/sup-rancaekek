...
if ($modul=='debit' AND $act=='hapus'){
            ...
            { 
                 ...
                 mysql_query("DELETE FROM rangkuman_aktifitas WHERE kode_nota='$_POST[kode_nota]' AND nama_irigasi = '$_SESSION[namairigasi]' AND nama_lengkap = '$_SESSION[namalengkap]'");
...
// Input tag
elseif ($modul=='debit' AND $act=='input'){
        ...
        if($count['no']!= $_POST['urutantanggal'])//untuk tidak memasukan no yang sama
        {
            ...
            { 
                 mysql_query("INSERT INTO `irigasi`.`rangkuman_aktifitas`(`modul`,`nama_irigasi`,`nama_lengkap`,`kode_nota`,`tanggal`,`bulan`,`tahun`,`ket`,`keterangan`,`tanggal_post`) VALUES ('$modul','$_SESSION[namairigasi]','$_SESSION[namalengkap]','$_POST[kode_nota]','$_POST[menutanggaket]','$_POST[menubulanket]','$_POST[menutahunket]','0','menginput data',CURRENT_TIMESTAMP)");//pemanggilan untuk update kegiatan
...
// Update tag
elseif ($modul=='debit' AND $act=='update'){
  mysql_query("UPDATE `blangko08` SET `id_nota` = '$_POST[id_nota]',
                                      ...
                                      `kode_irigasi` = '$_SESSION[kodeirigasi]',
            ...
            if($CountNo == $_POST['jumlahHari'])
            { 
                 $ket = mysql_query("SELECT MAX(ket)+1 AS KET FROM rangkuman_aktifitas WHERE nama_irigasi = '$_SESSION[namairigasi]' AND nama_lengkap = '$_SESSION[namalengkap]' AND  kode_nota = '$_POST[kode_nota]'");
                 $k    = mysql_fetch_array($ket);

                 $keterangankegiatan = "menyunting data";
                
                 mysql_query("UPDATE `irigasi`.`rangkuman_aktifitas` SET `ket`='$k[KET]',`keterangan`='$keterangankegiatan',`tanggal_post` = CURRENT_TIMESTAMP WHERE nama_irigasi = '$_SESSION[namairigasi]' AND nama_lengkap = '$_SESSION[namalengkap]' AND  kode_nota = '$_POST[kode_nota]'");//pemanggilan untuk update kegiatan
                 $count=mysql_fetch_array(mysql_query("SELECT * FROM debit_pertahun WHERE nama_irigasi = '$_SESSION[namairigasi]' AND tahun = '$_POST[menutahunket]'"));
                 if($count['tahun']== $_POST['menutahunket'])
                 {
